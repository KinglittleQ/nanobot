...
def get_context_window(model: str, model_config: dict[str, int] | None = None) -> int:
    ...
    Looks up *model_config* (substring-matched, case-insensitive).
    Falls back to DEFAULT_CONTEXT_WINDOW if not found.
    """
    if model_config:
...
def get_pricing(
    model: str,
    pricing_config: dict[str, list[float]] | None = None,
) -> tuple[float, float, float | None, float | None] | None:
    """Return (input, output, cache_write, cache_read) per MTok, or None if unknown.

    Looks up *pricing_config* (substring-matched, case-insensitive).
    Each value is a list of 2-4 floats: [input, output] or [input, output, cache_write, cache_read].
    """
    if pricing_config:
        model_lower = model.lower()
